...
resource "azurerm_key_vault_secret" "database_url" {
  name         = "databaseurl"
  value        = "postgresql://postgres:${azurerm_key_vault_secret.postgres_password.value}@${azurerm_postgresql_flexible_server.example.name}.${azurerm_private_dns_zone.dns_zone.name}:5432/career_journal?sslmode=require"
  key_vault_id = azurerm_key_vault.journal_kv.id
}
